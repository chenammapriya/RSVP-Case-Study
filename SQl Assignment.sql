...
SELECT     n.NAME AS actoress_name,
           Sum(r.total_votes) as total_votes,
           Count(r.movie_id)                                                                AS movie_count,
           Round(Sum(avg_rating*total_votes)/Sum(total_votes),2)                            AS actoress_avg_rating,
           Rank() OVER(ORDER BY Round(Sum(avg_rating*total_votes)/Sum(total_votes),2) DESC) AS actoress_rank
FROM       names n
INNER JOIN role_mapping rm
ON         n.id = rm.name_id
INNER JOIN ratings r
...
/* Based on number of movies, the top 9 directors are Andrew Jones, A.L. Vijay, 
Sion Sono, Chris Stokes, Sam Liu, Steven, Soderbergh, 
